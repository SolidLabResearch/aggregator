...
func TestRegistration_TokenUpdate_AuthorizationCode(t *testing.T) {
	...
	client := &http.Client{}
	startReq, err := http.NewRequest("POST", testEnv.AggregatorServerURL+"/registration", bytes.NewBuffer(body))
	if err != nil {
	...
	updateStartReq, err := http.NewRequest("POST", testEnv.AggregatorServerURL+"/registration", bytes.NewBuffer(updateStartJSON))
	if err != nil {
	...
	updateFinishReq, err := http.NewRequest("POST", testEnv.AggregatorServerURL+"/registration", bytes.NewBuffer(updateFinishJSON))
	if err != nil {
